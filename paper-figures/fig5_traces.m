...
% reproduce figure 5A,B (release and escape traces, ISI plots and spectrograms)
...
for i=1:2
    subplot(12,4,1+(i-1)*4), plot(x,smooth(data.V{1}(i,1:1+30*Fs),10),'k')
    xlim([0 x(end)]); ylim([-60 -18]);
    xticks([0:5:30]); yticks([-60:20:-20])
    if i==1; set(gca,'xtick',[]); title(['10' char(176) 'C']); end
    set(gca, 'Fontsize',12,'FontName','Arial'); box off; axis off
    h=display.plothorzline(data.Vth(1)); set(h,'color','r','linewidth',1)
end

% release at 20C
for i=1:2
    subplot(12,4,2+(i-1)*4), plot(x,smooth(data.V{1}(i,3*60*data.Fs:3*60*data.Fs+30*Fs),10),'k')
...
for i=1:2
    subplot(12,4,3+(i-1)*4), plot(x,smooth(data.V{2}(i,0.2*60*data.Fs:0.2*60*data.Fs+30*Fs),10),'k')
    xlim([0 x(end)]); ylim([-60 -20]);
    xticks([0:5:30]); yticks([-60:20:-20])
...
for i=1:2
    subplot(12,4,4+(i-1)*4), plot(x,smooth(data.V{2}(i,3.8*60*data.Fs:3.8*60*data.Fs+30*Fs),10),'k')
    xlim([0 x(end)]); ylim([-60 -20]);
    xticks([0:5:30]); yticks([-60:20:-20])
    if i==1; set(gca,'xtick',[]); end
    if i==1; set(gca,'xtick',[]); title(['20' char(176) 'C']); end
...
for j=1:2
    ...
    % whole trace
    for i =1:2
        subplot(6,2,3+(i-1)*2+j-1), plot(x,data.V{j}(i,:),'k')
        ylabel('V_M, mV');
    ...
    % saline temperature
    subplot(6,2,7+j-1), plot(x,data.T{j},'k','linewidth',2)
    ylabel('Temp, ^oC');
    set(gca,'xtick',[])
    set(gca, 'Fontsize',14,'FontName','Arial'); box off;
    ...
    subplot(6,2,9+j-1), plot(sthco{1}(1:end-1)/Fs/60,ISIhco,'.','markersize',5,'color','k')
    ...
    % Spectrogram
    n_sp=256; window = n_sp/10; % time window 
    [SP,f,time] = analysis.ARspectrogram(data.V{j}(1,:),data.Fs,window);
